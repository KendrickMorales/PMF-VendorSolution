...
def generate_base_part_number(file_path, existing_mappings):
    ...
    base_number = int(file_hash, 16) % 1000000000  # 9 digits from hash
...
def get_base_filename(file_path):
    ...
    # Also get directory path to ensure we match files in the same location
...
def get_or_create_part_mapping(file_path, existing_mappings, revision=None):
    ...
    if file_path in existing_mappings:
        ...
        # If revision specified and different, create new revision
        ...
        if matching_mapping:
            # Use the existing mapping from the file with same base name
            ...
            # Create new mapping
            # Note: generate_base_part_number uses base filename (without extension) for hash,
            # so files with same base name but different extensions will get the same part number
            base = generate_base_part_number(file_path, existing_mappings)
...
def read_file_metadata(file_path):
    """Read basic file metadata (fallback when SOLIDWORKS API not available)"""
    try:
    ...
    try:
        ...
        for file_info in file_paths:
            ...
            file_data = {
                ...
                'isRenamedFile': is_renamed_file,
                'originalFilenames': original_filenames,
                'note': 'SOLIDWORKS API not available - using file metadata only'
            }
            
            # Read basic file metadata
            metadata = read_file_metadata(file_path)
            if metadata:
                modified_date = datetime.fromtimestamp(metadata['modified']).strftime('%Y-%m-%d %H:%M:%S')
                file_size_mb = metadata['size'] / (1024 * 1024)
                file_data['properties'] = {
                    'File Size': {'value': str(metadata['size']), 'resolved': f"{file_size_mb:.2f} MB"},
                    'Modified Date': {'value': str(metadata['modified']), 'resolved': modified_date}
                }
...
def read_properties():
    """Read properties from SOLIDWORKS files (fallback mode)"""
    data = request.json
...
def update_properties():
    """Update vendor part numbers (stores in mapping, requires SOLIDWORKS to update files)"""
    data = request.json
    ...
    for update in updates:
        ...
        mappings[file_path] = {'base': vendor_part_number[:9], 'revision': int(vendor_part_number[9:])}
        results.append({
            'filePath': file_path,
            'success': True,
            'note': 'Mapping saved. Use SOLIDWORKS API or manual update to modify files.'
        })
